...
class PDFWatermarkRemover:
    def __init__(self, master):
        self.master = master
        self.master.title("PDF 水印去除工具 Mark6")
    ...
    def _update_mode_hint(self):
        ...
        if self.is_preview_session:
            ...
            self.mode_var.set(f"当前模式：原始文件（{name}）")
    ...
    def _find_rendered_diagonal_watermarks(self, page, page_idx: int) -> List[WatermarkInfo]:
        ...
        # The sample PDF uses a bottom-left to top-right watermark line.
        diagonal_angle = -0.85
        diagonal_intercept = height * 0.72
        diagonal_scale = math.sqrt(1 + diagonal_angle * diagonal_angle)
        ...
        for index in range(1, count):
            ...
            distance = abs(center_y - (diagonal_angle * center_x + diagonal_intercept))
            distance /= diagonal_scale
            if distance > height * 0.08:
                continue
            ...
            candidates.append((pdf_rect, area))
        ...
        rect = candidates[0][0]
        for candidate_rect, _ in candidates[1:]:
            rect |= candidate_rect
        ...
        return [
            WatermarkInfo(
                ...
                rect=rect,
                is_text=True,
                confidence=min(0.96, 0.72 + len(candidates) / 100.0),
                source="rendered-diagonal",
                occurrences=len(candidates),
                redaction_rects=[candidate_rect for candidate_rect, _ in candidates],
            )
        ]
    ...
    def _collect_vector_candidates(self, page, page_idx: int) -> List[VectorCandidate]:
        ...
        for item in page.get_drawings():
            ...
            dark_edge_rule = dark_color and near_edge and (horizontal_rule or vertical_rule)
    ...
    def _apply_watermarks_to_pdf(self, input_path: str, output_path: str, selected: List[WatermarkInfo]):
        ...
        try:
            ...
            for page_idx in range(work_doc.page_count):
                ...
                redact_needed = False
                for wm in page_items:
                    if wm.source == "rendered-diagonal":
                        continue
                    if wm.is_text:
