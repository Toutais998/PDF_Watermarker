...
class PDFWatermarkRemover:
    ...
    def _remove_watermarks_thread(self, output_path, selected_watermarks):
        """优化后的水印移除线程：采用底层资源剔除与无损擦除"""
        try:
            output_doc = fitz.open()
            
            # 优化点 1：全文档级别的 XRef (资源) 剔除
            # 针对图形、图像水印，直接清空其底层数据流，从根本上拔除且不留白块
            xrefs_to_kill = set()
            for watermark in selected_watermarks:
                if not watermark.is_text and watermark.index > 0:
                    xrefs_to_kill.add(watermark.index)

            for xref in xrefs_to_kill:
                try:
                    if self.doc.is_stream(xref):
                        # 将该对象的数据流替换为空，等同于使其隐形，完全不破坏PDF结构树
                        self.doc.update_stream(xref, b"")
                except Exception as e:
                    print(f"清理底层资源 {xref} 失败: {e}")

            is_modified = False
            total_pages = self.total_pages
            progress_step = max(1, total_pages // 10)

            # 优化点 2：逐页处理文本水印，启用图像保护机制
            for page_idx in range(total_pages):
                if page_idx % progress_step == 0:
                    progress_pct = int(page_idx / total_pages * 100)
                ...
                for watermark in page_watermarks:
                    # 收集需要擦除的文本水印区域
                    if watermark.is_text and watermark.rect:
                ...
                if rects_to_redact:
                    for rect in rects_to_redact:
                        # 核心修改：移除 fill=(1,1,1)，只标记擦除区域，不绘制遮盖用的白块
                        temp_page.add_redact_annot(rect, fill=None)
                    
                    # 核心修改：设置 images=fitz.PDF_REDACT_IMAGE_NONE
                    # 这确保了当文本水印与底层的显微图、数据图表重叠时，只擦除文字指令，绝不切碎或修改图像本身
            ...
            # 优化点 3：保存时启用高级垃圾回收 (garbage=3) 与重新压缩 (deflate=True)
            # 这会彻底清理掉第一步中变成孤岛的 XRef 资源，并显著减小最终文件体积
            if is_modified or xrefs_to_kill:
                ...
                self.master.after(0, lambda: messagebox.showinfo("成功", f"无损去水印完成，文件已保存至:\n{output_path}"))
            else:
                self.doc.save(output_path)
                self.master.after(0, lambda: self.status_var.set("未检测到需要处理的水印，已保存原件"))
