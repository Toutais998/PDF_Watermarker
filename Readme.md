...
- 本地 EXE：`dist\Mark8Final.exe`（需自行打包）
...
```powershell
.\.venv\Scripts\Activate.ps1
python -m PyInstaller --noconfirm --noconsole --onefile --clean --icon="pdf_tool_icon.ico" --name Mark8Final `
  ...
  --exclude-module tensorflow --exclude-module sklearn --exclude-module imageio_ffmpeg --exclude-module tables Mark8.py
```
...
- 新版本从上一版复制，例如 `Mark7.py` -> `Mark8.py`。
