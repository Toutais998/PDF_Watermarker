...
class PDFWatermarkRemover:
    ...
    def _build_ui(self):
        ...
        Button(top_frame, text="去除选中水印", bg=BUTTON_BG, fg=BUTTON_FG,
               command=self.remove_selected_watermarks, width=14, height=2).pack(side=LEFT, padx=5)
    ...
    def on_drop_file(self, event):
        ...
        if not file_path.lower().endswith(".pdf"):
            messagebox.showerror("文件类型错误", f"仅支持拖入 PDF 文件，收到：{os.path.basename(file_path)}")
            return
    ...
    def _analyze_watermarks_thread(self):
        try:
            ...
            for page_idx in range(self.total_pages):
                ...
                vector_candidates.extend(self._collect_vector_candidates(page, page_idx))
    ...
    def _find_text_watermarks(self, page, page_idx: int) -> List[WatermarkInfo]:
        ...
        for block in text_dict.get("blocks", []):
            ...
            for line in block["lines"]:
                ...
                large_center = rect and rect.width > page_width * 0.25 and rect.height < page_height * 0.2

                if keyword_hit or rotated or (in_edge and color_score) or (large_center and keyword_hit):
                    wm_type = "关键词文本" if keyword_hit else "旋转文本"
                    if in_edge and not keyword_hit and not rotated:
                        wm_type = "边缘文本"
                    results.append(
                        WatermarkInfo(
                            type_name=wm_type,
    ...
    def _collect_vector_candidates(self, page, page_idx: int) -> List[VectorCandidate]:
        ...
        for item in page.get_drawings():
            ...
            # 只保留疑似水印特征：大面积、浅色、或者半透明
            shallow_color = False
            color_value = fill or color
            if color_value and isinstance(color_value, tuple) and len(color_value) >= 3:
                shallow_color = sum(color_value[:3]) / 3 >= 0.72

            if area_ratio >= 0.02 and (opacity < 0.9 or shallow_color):
                slim = {
                    "type": item.get("type"),
                    "rect": tuple(round(v, 1) for v in rect),
    ...
    def remove_selected_watermarks(self):
        ...
        threading.Thread(
            target=lambda: self._remove_watermarks_thread(output_path, selected),
            daemon=True,
        ).start()

    def _remove_watermarks_thread(self, output_path: str, selected: List[WatermarkInfo]):
        try:
            ...
            # 1) 先移除重复图片资源
            for xref in xrefs_to_kill:
                try:
                    if work_doc.is_stream(xref):
                        work_doc.update_stream(xref, b"")
                except Exception:
                    pass
            ...
            work_doc.close()
            self.master.after(0, lambda: self.status_var.set(f"处理完成：{os.path.basename(output_path)}"))
            ...
            self.master.after(0, done_message)
        except Exception as e:
            self.master.after(0, lambda: messagebox.showerror("错误", f"去除水印失败: {e}"))
            self.master.after(0, lambda: self.status_var.set("去除水印失败"))
